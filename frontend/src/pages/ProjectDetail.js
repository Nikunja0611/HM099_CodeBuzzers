...
const ProjectDetails = () => {
  ...
  // --- IMPROVED LOCATION MATCHING ALGORITHM ---
  // Calculates a score (0-100) representing how close two locations are textually
  const getLocationScore = (loc1, loc2) => {
      ...
      // 2. Direct Inclusion (e.g., "Pune" in "Pune, India")
      if (s1.includes(s2) || s2.includes(s1)) return 80;

      // 3. Token/Word Match (e.g., "New Delhi" matches "Delhi")
      // Split by comma, space, or hyphen to get words
      const words1 = s1.split(/[\s,-]+/).filter(w => w.length > 2); // Ignore small words like "in", "at"
      const words2 = s2.split(/[\s,-]+/).filter(w => w.length > 2);

      const commonWords = words1.filter(word => words2.includes(word));
      
      // If they share words, give points. More shared words = higher score.
  ...
  const fetchData = async (isInitialLoad = false) => {
    try {
      ...
      // 2. Fetch Recommendations and Sort by Location Match
      if (isInitialLoad) {
          const recRes = await api.get(`/projects/${id}/recommendations`);
          let recs = recRes.data || [];

          // SORTING LOGIC: Prioritize Location Score, then AI Match Score
          if (projData.location) {
             recs = recs.map(rec => {
                 // Calculate score and attach it temporarily for sorting
                 return { ...rec, locationScore: getLocationScore(projData.location, rec.location) };
             }).sort((a, b) => {
                // Primary Sort: Location Score (Highest first)
                if (b.locationScore !== a.locationScore) {
                    return b.locationScore - a.locationScore;
                }
                // Secondary Sort: AI Match Score (Highest first)
  ...
  const generateDynamicInsights = (status, progressPct) => {
    ...
    const resources = project.resource_availability || 'Medium';
    ...
    if (diff > 10) {
        analysis.push(`You are ${Math.round(diff)}% ahead of schedule.`);
    } else if (diff < -15) {
        analysis.push(`Falling behind: ${cleanTime}% time passed but only ${cleanProgress}% complete.`);
        actions.push("Re-evaluate timeline or reduce scope.");
    } else {
        analysis.push(`On track: Progress aligns with timeline.`);
    }

    if (budget > progressPct + 20) {
        analysis.push(`Overspending detected: Budget used (${budget}%) exceeds progress.`);
    }
  ...
  if (project.confidence !== undefined && project.confidence !== null) {
      globalConfidence = Number(project.confidence);
      if (globalConfidence > 0 && globalConfidence <= 1) globalConfidence *= 100;
  }
           ...
           <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
              ...
              <div className="flex flex-wrap gap-4">
                 {sdgList.map((sdgNum, idx) => {
                    ...
                    if (project.sdg_scores && project.sdg_scores[String(sdgNum)]) {
                        specificConf = project.sdg_scores[String(sdgNum)];
                    }
                    return (
                        <div key={idx} className="bg-gray-50 rounded-xl p-6 flex flex-col items-center justify-center min-w-[140px] border border-gray-100 flex-1">
                           <div className="w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl mb-3 shadow-md" style={{ backgroundColor: SDG_COLORS[sdgNum] || '#19486A' }}>
                              {sdgNum}
                           </div>
                           <h4 className="font-bold text-gray-900 text-sm">{SDG_LABELS[sdgNum] || `Goal ${sdgNum}`}</h4>
                           <p className="text-xs text-green-600 font-bold mt-1">{Number(specificConf).toFixed(0)}% Confidence</p>
                        </div>
                    );
                 })}
              </div>
           ...
           <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
              ...
              <div className="space-y-4">
                 {(!project.milestones || project.milestones.length === 0) ? (
                    ...
                    <div key={i} onClick={() => handleToggleMilestone(i)} className={`flex items-center p-4 rounded-xl border cursor-pointer transition-all ${ms.completed ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200 hover:border-teal-300'}`}>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {recommendations.length === 0 ? (
                      <div className="col-span-2 p-6 bg-white rounded-xl text-center text-gray-400 text-sm border border-dashed border-gray-200">
                          {project.sdg ? "AI is searching..." : "Add SDGs to get partner recommendations."}
                      </div>
                  ) : recommendations.map((rec) => {
                      // Check relevance score > 50 to call it a match
                      const locScore = getLocationScore(project.location, rec.location);
                      const isLocalMatch = locScore > 50; 
                      ...
                      return (
                        <div key={rec._id} className={`bg-white rounded-xl p-5 shadow-sm border hover:shadow-md transition-all h-full flex flex-col ${isLocalMatch ? 'border-teal-300 ring-1 ring-teal-100' : 'border-gray-200'}`}>
                            ...
                            {isLocalMatch && (
                                <div className="mb-2">
                                    <span className="bg-teal-100 text-teal-800 text-[10px] font-bold px-2 py-1 rounded flex items-center w-fit gap-1">
                                        <MapPin size={10} fill="currentColor" /> Local Partner Match
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex gap-3">
                                    ...
                                    <div className="flex items-center gap-1 text-xs text-gray-500 mt-1">
                                        <MapPin size={10} className={isLocalMatch ? "text-teal-600" : ""}/> 
                                        <span className={isLocalMatch ? "font-bold text-teal-700" : ""}>{rec.location || 'Global'}</span>
                                    </div>
                                    </div>
                                </div>
                            ...
                            {rec.interests && (
                                <div className="mb-4">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {rec.interests.split(',').slice(0, 3).map((interest, i) => (
                                            <span key={i} className="px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] rounded font-medium border border-blue-100">
                                                {interest.trim()}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="bg-green-50 rounded-lg p-4 mb-4 border border-green-100">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className="text-lg font-black text-green-600">{rec.match_score ? `${rec.match_score}%` : '—'}</span>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    {isLocalMatch && (
                                        <div className="flex items-center gap-1.5 text-[10px] text-teal-700 font-bold">
                                            <CheckCircle size={10} className="text-teal-600"/> Location Optimized
                                        </div>
                                    )}
                                </div>
                            </div>
           ...
           {analyzing ? (
              ...
              <div className={`rounded-2xl p-6 shadow-sm border ${statusStyle.borderColor} bg-white relative overflow-hidden transition-all duration-500 ease-in-out`}>
                  ...
                  <div className="flex flex-col items-center text-center mb-6">
                      <div className={`w-20 h-20 ${statusStyle.bgColor} rounded-full flex items-center justify-center ${statusStyle.color} mb-3 shadow-inner`}>
                        {statusStyle.icon}
                      </div>
                      <h4 className={`text-2xl font-black ${statusStyle.color}`}>{currentStatus}</h4>
                      <div className="flex items-center gap-1 mt-1 bg-gray-100 px-3 py-1 rounded-full">
                        <Zap size={10} className="text-gray-400"/>
                        <p className="text-xs text-gray-500 font-bold">{currentConfidence.toFixed(1)}% Confidence Model</p>
                      </div>
                  </div>
                  <div className="mb-6">
                      <h5 className="text-xs font-bold text-gray-800 uppercase mb-3 flex items-center gap-1">
                        <Info size={12} className="text-gray-400"/> Why this status?
                      </h5>
                      {analysis.length > 0 ? (
                          <div className="space-y-2">
                            {analysis.map((point, i) => (
                                <div key={i} className="flex items-start gap-2 text-xs text-gray-600 leading-snug">
                                  <CheckCircle size={12} className={`mt-0.5 shrink-0 ${statusStyle.color}`}/> 
                                  <span>{point}</span>
                                </div>
                            ))}
                          </div>
                  ...
                  {actions.length > 0 && (
                    <div className={`rounded-xl p-4 ${statusStyle.bgColor} border ${statusStyle.borderColor}`}>
                        <h5 className={`text-xs font-bold uppercase mb-2 ${statusStyle.color}`}>Recommended Actions</h5>
                        <ul className="space-y-2">
                          {actions.map((action, i) => (
                              <li key={i} className="text-xs text-gray-700 flex items-start gap-2">
                                <div className={`w-1 h-1 rounded-full mt-1.5 shrink-0 bg-gray-400`}></div>
                                {action}
                              </li>
                          ))}
                        </ul>
